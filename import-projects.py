...
--------------
    ...
    Redirect 301 /nl/projects/{id}-{slug} https://except.eco/nl/projects/{slug}/
...
author = People.objects.get(pk=1)
main = ProjectIndexPage.objects.all()[0]
i = 1
...
[article.delete() for article in ProjectPage.objects.all() if not article.body]
...
with open('main_asset.csv') as csv_file:
    ...
    for row in csv_reader:
        if row['status'] == 'p' and row['assettype'] == 'Project':
            ...
            try:
                post = ProjectPage(  # .objects.create(
                    author=author,
                ...
                # markdown code
                old_site = 'https://except.nl/en/projects/%s-%s' % (row['id'], row['slug'])
                old_site_admin = 'http://except.nl/admin/main/project/%s/' % row['id']
                html_en = 'file:///C:/Users/luzdealba/OneDrive/Work/Except/backups/projects/%s_en.txt' % row['id']
                html_nl = 'file:///C:/Users/luzdealba/OneDrive/Work/Except/backups/projects/%s_nl.txt' % row['id']
                new_site = 'https://except.eco/projects/%s/' % row['slug']
