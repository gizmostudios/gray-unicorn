...
main = NewsIndexPage.objects.all()[0]
i = 1
...
[article.delete() for article in NewsPage.objects.all() if not article.body]
    ...
    for row in csv_reader:
        if row['status'] == 'p' and row['assettype'] == 'NewsItem':
            ...
            try:
                ...
                # markdown code
                old_site = 'https://except.nl/en/news/%s-%s' % (row['id'], row['slug'])
                old_site_admin = 'http://except.nl/admin/main/newsitem/%s/' % row['id']
                html_en = 'file:///C:/Users/luzdealba/OneDrive/Work/Except/backups/news/%s_en.txt' % row['id']
                html_nl = 'file:///C:/Users/luzdealba/OneDrive/Work/Except/backups/news/%s_nl.txt' % row['id']
                new_site = 'https://except.eco/about/news/excepts-news/%s/' % row['slug']
