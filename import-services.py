...
--------------
    ...
    Redirect 301 /en/services/{id}-{slug} https://except.eco/services/{slug}/
    Redirect 301 /nl/services/{id}-{slug} https://except.eco/nl/diensten/{slug}/
...
from services.models import ServicePage, ServiceIndexPage
from people.models import People

author = People.objects.get(pk=5)
main = ServiceIndexPage.objects.all()[0]
i = 1
...
[article.delete() for article in ServicePage.objects.all() if not article.body]
...
with open('main_asset.csv') as csv_file:
    ...
    for row in csv_reader:
        if row['status'] == 'p' and row['assettype'] == 'Service':
            ...
            try:
                post = ServicePage(  # .objects.create(
                    # author=author,
                    ...
                    # header_title=row['p_title'],
                    # header_title_en=row['p_title_en'],
                    # header_title_nl=row['p_title_nl'],
                    # header_subtitle=row['p_subtitle'],
                    # header_subtitle_en=row['p_subtitle_en'],
                    # header_subtitle_nl=row['p_subtitle_nl'],
                ...
                # markdown code
                old_site = 'http://except.nl/en/services/%s-%s' % (row['id'], row['slug'])
                old_site_admin = 'http://except.nl/admin/main/service/%s/' % row['id']
                html_en = 'file:///C:/Users/luzdealba/OneDrive/Work/Except/backups/services/%s_en.txt' % row['id']
                html_nl = 'file:///C:/Users/luzdealba/OneDrive/Work/Except/backups/services/%s_nl.txt' % row['id']
                new_site = 'https://except.eco/services/%s/' % row['slug']
