...
$(document).ready(function() {
  ...
  $('#video-control').on('click', function() {
    var button = $(this);
    var video = document.getElementById('video-controlled');
    if (video) {
      if (video.paused) {
        button.toggleClass('video-control-visible');
        $('i', button).removeClass('fa-play-circle').addClass('fa-pause-circle');
        video.play();
      } else {
        button.toggleClass('video-control-visible');
        $('i', button).removeClass('fa-pause-circle').addClass('fa-play-circle');
        video.pause();
      }
    }
  });
  ...
  // HomePage carousels:
  if ($('.home-first-glide').length) {
    new Glide('.home-first-glide', {
      type: 'carousel',
      autoplay: 6000
    }).mount();
  }
  ...
  if ($('[data-event="load-project"]').length) {
    $('[data-event="load-project"]').on('click', function() {
      $('.timeline .project.active').removeClass('active');
      $(this).addClass('active');
      $.ajax({
        url: $(this).data('url'),
        method: 'GET',
        success: function(data) {
          $('.selected-project').hide().html(data.html).fadeIn('slow');
        }
      });
    });
  }
  ...
  if ($('#nav-next .pagination-link-button').length) {
    window.addEventListener('scroll', function (e) {
      if (isVisible(document.getElementById('nav-next'))) {
        $('#nav-next .pagination-link-button').click();
      }
    });
  }
