...
# from news.models import NewsPage, NewspaperArticlePage
# from people.models import ProfilePage
...
def search(request):
    ...
    if search_query:
        # if 'elasticsearch2' in settings.WAGTAILSEARCH_BACKENDS['default'][
        #     'BACKEND'
        # ]:
        #     # In production, use ElasticSearch and a simplified search query,
        #     # per http://docs.wagtail.io/en/v1.12.1/topics/search/backends.html
        #     # like this:
        #     search_results = Page.objects.live().search(search_query)
        # else:
        #     # If we aren't using ElasticSearch for the demo, fall back to
        #     # native db search.
        #     # But native DB search can't search specific fields in our models
        #     # on a `Page` query.
        #     # So for demo purposes ONLY, we hard-code in the model names
        #     # we want to search.
        ...
        #     newspaper_news_results = NewspaperArticlePage.objects.live(
        #     ).search(search_query)
        #     newspaper_news_ids = [
        #         p.page_ptr.id for p in newspaper_news_results
        #     ]
        ...
        #     page_ids = (
        #         article_page_ids + article_page_ids + newspaper_news_ids +
        #         people_ids + project_ids + service_ids
        #     )
